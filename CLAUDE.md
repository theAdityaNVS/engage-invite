...
- `src/pages/_app.js` — global provider wrapper (LanguageProvider + Lenis smooth scroll init)
...
- `src/components/shared/` — used across both sites (SplashScreen, LanguageModal, MusicPlayer, ProgressDots, dividers, etc.)
- `src/components/engagement/` — specific to `/engagement` page

### Key Shared Components

- `SplashScreen` — temple-door animation + language picker + "Tap to open" button; skips language picker if `hasStoredLang` is true
- `ProgressDots` — fixed right-side dot nav; tracks active section via IntersectionObserver
- `AutoScrollHint` — chevron that appears after 10s idle, dismissed on any scroll
- `GarlandDivider`, `MangoToranDivider`, `KolamDivider` — decorative SVG section dividers; accept `fromColor`/`toColor` props
...
- `public/illustrations/` — `jagannath-temple-day.png`, `tirupati-gopuram-night.png`, `vintage-car-blue-mercedes.png`
- OG image expected at `public/og-engagement.jpg` (not yet provided)
